...
class RPlanhgDataset(Dataset):
    def __init__(self, set_name, analog_bit, target_set, non_manhattan=False):
        ...
        if os.path.exists(f'processed_rplan/rplan_{set_name}_{target_set}.npz'):
            ...
            for h, graph, path in tqdm(zip(self.org_houses, self.org_graphs, valid_filenames), desc='processing dataset'):
                house = []
                corner_bounds = []
                num_points = 0
                for i, room in enumerate(h):
            ...
            print(len(filenames), len(valid_filenames), len(in_files))
            np.savez_compressed(f'processed_rplan/rplan_{set_name}_{target_set}', graphs=np.array(self.graphs, dtype=object), houses=self.houses,
                    door_masks=self.door_masks, self_masks=self.self_masks, gen_masks=self.gen_masks, filenames=np.array(in_files))
            if self.set_name=='train':
            ...
            if set_name=='eval':
                ...
                np.savez_compressed(f'processed_rplan/rplan_{set_name}_{target_set}_syn', graphs=np.array(self.syn_graphs, dtype=object), houses=np.array(self.syn_houses, dtype=object), door_masks=self.syn_door_masks, self_masks=self.syn_self_masks, gen_masks=self.syn_gen_masks)
